...
void iterate_allan(Device*);
void compute_allan_(FILE *, FILE *);
...
void compute_allan_(FILE *in, FILE *out)
{
	...
	int clusts;							// The number of clusters to be created.
	int size;								// The size of in.
	float sum_sd;						// Sum of the squared differences.
	float sum_cluster;			// Holds the sum of clustered values.
	float* in_values;

	// Creates the memory map for the floats values.
	in_values = mmap(NULL,get_size(in),PROT_READ,MAP_SHARED,fileno(in),0);
  // Computes the number of floats in in.
	size = get_size(in)/sizeof(float);
	// Computes the maximum number of iterations.
	max_i = log(size/2.0)/log(2.0);
	...
	for(i=0;i<max_i;i++)
	{
		...
		tau = pow(2,i+1);
		// Computes the number of clusters of size tau.
		clusts = size/tau;
		sum_sd = 0;
